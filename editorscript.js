...
// Simple Methods
    ...
    function isUnderPanel(panelUnder, panelAbove){
        return Math.abs(panelUnder.x - panelAbove.x) < panelUnder.width - 3 && Math.abs(panelUnder.y - panelAbove.y) < panelUnder.height - 3;
    }
...
// Add Button
    ...
    buttonSprite.on('pointerup', (event) => {
        ...
        if (filename) {
            var gamePanelString = JSON.stringify(gamePanelConfig);
